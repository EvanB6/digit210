...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    *  Portions of the stacked plot would be count-of-moves-in-each-category div total count * 100
    ...
    <xsl:variable name="distcategory" as="xs:string+" select="category => distinct-values()"/>        
               <xsl:variable name="sortedCategory" as="xs:string+">
                   <xsl:for-each select="$distcategory">
                       <xsl:sort select="$pokemonCategories" order = "descending">
